...
## 十一、Gemini 補充意見（2026-04-01）

針對 AWS 專案的三個核心機制，Gemini 提出以下深化觀點，並對應個金 Co-Pilot 的落地方向：

---

### 機制一：`input_mode` — 強制人工確認節點

**AWS 設計**：在 Agent 執行關鍵 Task（例如生成最終理賠給付通知書）之前，強制跳出確認介面。人類核保師可對 AI 草稿進行**即時修正**——例如 AI 漏掉除外責任，人類在對話框輸入修正建議，Agent 重新計算結果。

**個金 Co-Pilot 現狀**：
目前行員決策僅有「核准 / 婉拒」二元選擇，批覆書參數無法調整。
...
AI 產出批覆書草稿
    ↓
[強制介入點] 行員確認介面
    ├── 核准金額：AI 建議 800 萬 → 行員可調整為 700 萬
    ├── 利率：AI 建議 2.06% → 行員可覆蓋
    └── 附加條件：行員可新增「需提供擔保人」等條件
    ↓
行員確認後 → 發送 LINE 通知給客戶
```

**對應位置**：`AdminCaseDetailPage` 的「行員最終決策」區塊需新增參數調整表單。

---

### 機制二：異常路由（Escalation Routing）— 三級分派架構

**AWS 設計**：Orchestrator 依風險等級自動路由：
- **Level 1（AI Agent）**：處理 80% 標準件（小額、資料齊全、低風險）
- **Level 2（Human Supervisor）**：文件模糊、疑似詐欺、資訊矛盾 → 直接路由給人類

**對照 Crew 7 防詐現有邏輯**：

```
目前做法：防詐分數 → 顯示標籤（低/中/高風險）→ 行員自行判斷

應強化為三級路由：
防詐分數 ≤ 40   →  Level 1：AI 自動建議核准，行員一鍵確認
防詐分數 41–70  →  Level 2：標示「需人工審查」，指派資深行員
防詐分數 > 70   →  Level 3：凍結案件，強制主管介入 + 紅色警告
```

**對應位置**：`AdminCasesPage` 案件列表需新增分級視圖；`AdminDashboardPage` 新增「待人工審核」獨立區塊。

---

### 機制三：Reviewer Portal — 獨立人工審核入口

**AWS 設計**：專屬 `/reviewer` Portal 接收被 AI 標記為「待人工審核」的案件，與一般案件列表分離，避免混雜。

**個金 Co-Pilot 現狀**：所有案件（含 AI 標記高風險）混在同一個 `AdminCasesPage`，無獨立審核視圖。

**最簡落地方式（不需新頁面）**：
在現有案件列表新增 `ai_flagged` 狀態欄；`AdminDashboardPage` 新增「AI 標記待審」快速入口卡片。
...
| 機制 | 實作難度 | 黑客松衝擊力 | 建議時程 |
|------|---------|------------|---------|
| 異常路由三級分層 | 低（改 UI 邏輯）| ⭐⭐⭐⭐⭐ | **立即實作** |
| `input_mode` 批覆書參數修正 | 中（加表單欄位）| ⭐⭐⭐⭐⭐ | 黑客松前 |
| 獨立 Reviewer Portal | 高（新頁面）| ⭐⭐⭐ | 黑客松後 |

...
### 投影片 2【三大斷點 + 機會規模】
標題：個金業務三大斷點：現有系統的邊界，就是 AI 介入的起點

三大斷點（前中後端框架，精簡版）：

📱 前端（獲客斷點）：數位體驗斷裂與高齡落差
現況：LINE Bot 僅是導流工具（點即跳轉），400 萬高齡長輩無法適應純 App
痛點：缺乏自然語言互動，服務在「進件前」就已流失

⚙️ 中端（營運斷點）：規則引擎（IF/THEN）撞上天花板
現況：E-LOAN 處理標準案件，但灰色地帶 5P 徵審 / 動態鑑價 / 非結構化法規查詢完全失效
痛點：高度依賴人工判斷，審件上限死鎖在 40 件/月

📁 後端（資料斷點）：資料孤島與缺乏 AI-readability
現況：分行回報依賴 Excel / PDF，法規公文散落各處
痛點：資料無法轉化為 ML 訓練語料，AI 無法持續進化——這是根本問題
...
【口述補充】：三個斷點分別對應 Co-Pilot 三個核心解法——
前端 → Crew 5 台語語音 + LINE 完整申貸；
中端 → Crew 1–4 多代理並行徵審；
後端 → MD+YAML 知識金庫，資料直接成為 ML 語料。
